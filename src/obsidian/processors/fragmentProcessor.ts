...
export class FragmentProcessor {
    ...
    process(markdown: string, options: Options) {
        ...
        const output = markdown
            ...
            .map((line, lineNumber) => {
                ...
                const isCodeblockLine = codeBlockLines.some(
    ...
    transformLine(line: string) {
        const comment =
            this.parser.parseLine(line) ?? this.parser.buildComment('element');
        ...
        const extra_replacement =
            '&shy;' + this.parser.commentToString(comment);
